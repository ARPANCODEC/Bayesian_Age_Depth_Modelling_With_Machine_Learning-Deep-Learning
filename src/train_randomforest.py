...
# Save scaler
joblib.dump(scaler, os.path.join(MODEL_DIR, "randomforest_scaler.pkl"))
...
results_path = os.path.join(OUTPUT_DIR, "randomforest_predictions.csv")
...
metrics_df.to_csv(os.path.join(OUTPUT_DIR, "randomforest_metrics.csv"), index=False)
...
plt.tight_layout()
plot_path = os.path.join(OUTPUT_DIR, "randomforest_predictions_scatter.png")
plt.savefig(plot_path, dpi=300, bbox_inches='tight')
plt.close()

# Feature importance plot
plt.figure(figsize=(8, 6))
plt.barh(FEATURES, feature_importance, color='lightgreen', edgecolor='darkgreen')
plt.xlabel('Importance')
plt.title('Random Forest Feature Importance')
plt.grid(True, alpha=0.3)
importance_path = os.path.join(OUTPUT_DIR, "randomforest_feature_importance.png")
...
print(f"   • Scaler: {os.path.join(MODEL_DIR, 'randomforest_scaler.pkl')}")
print(f"   • Predictions: {results_path}")
print(f"   • Metrics: {os.path.join(OUTPUT_DIR, 'randomforest_metrics.csv')}")
